...
class Dynamic: # Linear
    def __init__(self, state_dim: int, control_dim: int):
        self.state_dim = state_dim; self.control_dim = control_dim
        self.tau_dim = state_dim + control_dim

    def step(self, x_init: Tensor, u: Tensor) -> Tensor: # (s, ), (u, )
        if len(x_init.shape) == 1: x_init = x_init.unsqueeze(dim=-1)
        if len(u.shape) == 1: u = u.unsqueeze(dim=-1)
        F, f = self._get_dynamic_param()
        tau = torch.concat([x_init, u], dim=0) # (c, 1)
        x = F @ tau + f
        return x.squeeze() # (s, )
    
    def forward(self, x_init: Tensor, u: Tensor) -> Tensor: # (s, ), (T, u)
        T: int = u.shape[0]
        if len(x_init.shape) == 1: x_init = x_init.unsqueeze(dim=-1)

        x: Tensor = torch.zeros(T+1, self.state_dim)
        xt = x_init.clone(); x[0, :] = xt.flatten()
        for t in range(T):
            xt = self.step(xt, u[t]) # (s, 1)
            x[t+1, :] = xt.flatten()
        return x # (T, s)

    def _get_dynamic_param(self) -> Tuple[Tensor, Tensor]:
        F = torch.rand(self.state_dim, self.tau_dim) # (s, c)
        f = torch.rand(self.state_dim, 1) # (s, 1)
        return F, f
...
class Cost:
    ...
    def step(self, x: Tensor, u: Tensor) -> float: # (s, ), (u, )
        if len(x.shape) == 1: x = x.unsqueeze()
        if len(u.shape) == 1: u = u.unsqueeze()
        C, c = self._get_cost_param()
        tau: Tensor = torch.concat([x, u], dim=0) # (c, 1)
        obj = 0.5 * tau.T @ C @ tau + c.T @ tau
        return obj.item()
    
    def get_obj(self, x: Tensor, u: Tensor) -> float: # (T, s), (T, u)
        T: int = x.shape[0]
        objs: float = 0.0
        for t in range(T):
            objs += self.step(x[t], u[t])
        return objs

    def _get_cost_param(self) -> Tuple[Tensor, Tensor]:
        C = torch.rand(self.tau_dim, self.tau_dim) # (c, c)
        c = torch.rand(self.tau_dim, 1)  # (c, 1)
        return C, c
