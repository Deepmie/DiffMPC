...
class BaseDynamic(ABC, nn.Module):
    ...
    @abstractmethod
    def forward(self, x: Tensor, u: Tensor, t: Optional[int]=None) -> Tensor: # (b, s), (b, u)
        return torch.rand(x.shape) # (b, s)
    
    def step(self, u: Tensor, x_init: Tensor, t: Optional[int]=None) -> Tensor: # (b, u), (b, s)
        x_next: Tensor = self.forward(x_init, u, t) # (b, s)
        return x_next

    def get_traj(self, u: Tensor, x_init: Tensor): # (T, b, u), (b, s)
        T = u.shape[0]; batch_size = u.shape[1]; state_dim = x_init.shape[1]
        x: Tensor = torch.zeros(T, batch_size, state_dim, dtype=x_init.dtype)
        x[0] = x_init
        for t in range(T-1): x[t+1] = self.step(u[t], x[t], t)
        return x # (T, b, u)
...
class AffineDynamics(BaseDynamic):
    ...
    def forward(self, x: Tensor, u: Tensor, t: Optional[int]=None) -> Tensor: # (b, s), (b, u)
        if x.ndimension() == 1: x = x.unsqueeze(dim=0)
        if u.ndimension() == 1: u = u.unsqueeze(dim=0)
        z: Tensor = x @ self.A.T + u @ self.B.T + self.c # (b, s)
        return z # (b, s)

    def grad_input(self, x: Tensor, u: Tensor) -> Tuple[Tensor, Tensor]: # (xx, s), (xx, u)
...
class LinDynamics(BaseDynamic):
    def __init__(self, F: Tensor, f: Optional[Tensor]=None): # (T-1, b, s, c), (T-1, b, s)
        super(LinDynamics, self).__init__()
        self.F = F; self.f = f

    def forward(self, x: Tensor, u: Tensor, t: int) -> Tensor: # (b, s), (b, u)
        if x.ndimension() == 1: x = x.unsqueeze(dim=0)
        if u.ndimension() == 1: u = u.unsqueeze(dim=0)
        tau: Tensor = torch.concat([x, u], dim=-1) # (b, c)
        z: Tensor = (self.F[t] @ tau.unsqueeze(dim=-1)).squeeze() # (b, s)
        if self.f is not None: z += self.f[t] # (b, s)
        return z

    def grad_input(self, x: Tensor, u: Tensor) -> None:
        ...

    def get_linear_params(self, x: Tensor, u: Tensor) -> Tuple[Tensor, Tensor]: # (T, b, s), (T, b, u)
        return self.F, self.f # (T-1, b, s, c), (T-1, b, s)
...
class QuadCost:
    ...
    def get_objs(self, x: Tensor, u: Tensor) -> Tensor: # (T, b, s), (T, b, u)
        tau: Tensor = torch.concat([x, u], dim=-1) # (T, b, c)
        objs: Tensor = 0.5 * (tau.unsqueeze(dim=-2) @ self.C @ tau.unsqueeze(dim=-1)).squeeze() + (self.c.unsqueeze(dim=-2) @ tau.unsqueeze(dim=-1)).squeeze() # (T, b)
        return objs # (T, b)

    def get_costs(self, x: Tensor, u: Tensor) -> Tensor: # (T, b, s), (T, b, u)
        objs = self.get_objs(x, u) # (T, b)
